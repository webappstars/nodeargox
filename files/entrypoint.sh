  ...
  cat > config.json << EOF
{
  ...
  "inbounds": [
    ...
    },
    {
      ...
      "streamSettings": {
        "network": "ws",
        "wsSettings": { "path": "/${WSPATH}-shadowsocks" }
      },
      "sniffing": {
        "enabled": true,
        "destOverride": ["http", "tls"],
...
download_agent() {
  if [ ! -e nezha-agent ]; then
    URL=\${URL:-https://github.com/nezhahq/agent/releases/download/v0.20.5/nezha-agent_linux_amd64.zip}
    wget \${URL}
